...
export class ForceComponent implements OnInit {
  ...
  async graphData(universitys:any,researchers:any[],years:number[]){
    ...
    result.records.forEach(r => { 
      duplicateNodes.push({id: r.get('a1').identity.low, label: r.get('a1').properties.name,size:r.get('a1').properties.louvain_undirected});
      duplicateNodes.push({id: r.get('a2').identity.low, label: r.get('a2').properties.name,size:r.get('a2').properties.louvain_undirected});
    });
